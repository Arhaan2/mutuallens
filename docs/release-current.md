...
Hosted stamp checks accept `MUTUALLENS_EXPECTED_SHA` for the Cloudflare application and `MUTUALLENS_EXPECTED_PROJECT_SHA` for the GitHub project (defaults to the application SHA). They also require all three hosts to share the same application source hash. This preserves exact source checks after a documentation-only Pages rebuild.
